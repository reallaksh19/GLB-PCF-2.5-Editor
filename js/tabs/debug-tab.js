...
function renderActiveSection() {
  const container = byId('debug-content');
  if (!container) return;
...
function syncSectionButtons() {
  document.querySelectorAll('[data-debug-section]').forEach((btn) => {
    btn.classList.toggle('active', btn.dataset.debugSection === debugState.activeSection);
  });
}

export function initDebugTab() {
  const domainLabel = byId('debug-domain-label');
  const refreshBtn = byId('btn-debug-refresh');
  const copyBtn = byId('btn-debug-copy-json');
  const exportBtn = byId('btn-debug-export-log');

  document.querySelectorAll('[data-debug-section]').forEach((btn) => {
    btn.addEventListener('click', () => {
      debugState.activeSection = btn.dataset.debugSection || 'summary';
      syncSectionButtons();
      renderActiveSection();
    });
  });
