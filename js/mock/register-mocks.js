...
capabilities.registerMock('component-panel', async () => {
  ...
  if (sections.length) {
    ...
    const container = document.getElementById('hifi-viewer-inspector');
...
capabilities.registerMock('css2d-labels', async () => {
  ...
  const canvasDiv    = document.getElementById('hifi-viewer-canvas');
...
capabilities.registerMock('debug-tab', async () => {
  ...
  // Read debug content
  const content     = document.getElementById('hifi-debug-content')?.textContent || '';
  const badgeText   = document.getElementById('hifi-debug-domain-label')?.textContent || '';

  // Switch to summary section to get the rendered data
  document.querySelector('[data-hifi-debug-section="summary"]')?.click();
  await new Promise(r => setTimeout(r, 100));
  const summaryText = document.getElementById('hifi-debug-content')?.textContent || '';
  ...
  document.querySelector('[data-hifi-debug-section="validation"]')?.click();
  await new Promise(r => setTimeout(r, 100));
  const validText   = document.getElementById('hifi-debug-content')?.textContent || '';
