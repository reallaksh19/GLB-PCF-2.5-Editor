...
import { parseDxf, parseDxfWithCeg } from './dxf-importer.js';
...
export const domain = {
  ...
  /**
   * Parse DXF and return both components and a CEG.
   * @returns {{ components: Array, ceg: Object|null }}
   */
  parseDxfWithCeg(text, log) {
    if (!text) {
      log.error('PARSE_FAIL', { message: 'Input text is empty' });
      return { components: [], ceg: null };
    }
    return parseDxfWithCeg(text, log);
  },

  buildMesh(comp, theme, options = {}) {
    return buildMesh(comp, theme, options);
  },
