...
import { pcfComponentsToCeg } from '../../formats/pcf/pcf-to-ceg.js';
...
export function parsePcf(text, log) {
  ...
  try {
    ...
    const rawComponents = (model.components || []).map(comp => toGenericComponent(comp));
    const { graph, components } = pcfComponentsToCeg(rawComponents, { name: 'PCF Import' });

    log.info('PCF_PARSE_DONE', {
      componentCount: components.length,
      cegComponentCount: Object.keys(graph.components || {}).length,
      anchorCount: Object.keys(graph.anchors || {}).length,
